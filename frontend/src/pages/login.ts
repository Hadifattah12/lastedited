...
import { apiFetch } from '../utils/api';     // BACKEND_BASE no longer needed here

export function renderLogin(): HTMLElement {
  const container = document.createElement('div');
  document.body.className = 'login-page';

  /* ---------------- Google-OAuth error in query-string ---------------- */
  ...
  container.innerHTML = `
    <div class="ring">
      ...
      <div class="login">
        ...
        <div class="inputBx"><input id="loginBtn" type="submit"   value="Sign in"></div>
        ...
        <div class="links"><a class="signup" href="#/signup">Signup</a></div>
      </div>
    </div>
  `;
  ...
  loginBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (data.user.is2FAEnabled) {
        ...
        location.hash = '#/home';                   // cookie already set
      }
    } catch (err) {
      console.error(err);
